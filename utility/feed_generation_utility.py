...
def train_batch_generator_408(batch_size=64):
    ...
    while True:
        if start_idx >= len(train_lengths):
            start_idx = 0
        ...
        W = [
            [
                1 if l < train_lengths[idx] else 0
                for l in range(
                    0,
                    train_lengths[
                        min(start_idx + batch_size, len(train_lengths)) - 1
                    ],
                )
            ]
            for idx in range(
                start_idx, (min(start_idx + batch_size, len(train_lengths)))
            )
        ]
...
def validation_batch_generator_408(batch_size=100):
    ...
    while True:
        ...
        W = [
            [
                1 if l < test_lengths[idx] else 0
                for l in range(
                    0,
                    test_lengths[
                        min(start_idx + batch_size, len(test_lengths)) - 1
                    ],
                )
            ]
            for idx in range(
                start_idx, (min(start_idx + batch_size, len(test_lengths)))
            )
        ]
        start_idx += batch_size
        yield X, Y, np.array(W)
        ...
        W = [
            [
                ...
                for l in range(
                    0,
                    test_lengths[
                        min(start_idx + batchsize, len(test_lengths)) - 1
                    ],
                )
            ]
            for idx in range(
                start_idx, (min(start_idx + batchsize, len(test_lengths)))
            )
        ]
        start_idx += batchsize
        yield X, Y, np.array(W)
