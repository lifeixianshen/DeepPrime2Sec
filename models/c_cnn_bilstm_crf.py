...
def model_c_cnn_bilstm_crf(n_classes, convs=[3, 5, 7], dense_size=200, lstm_size=400, dropout_rate=0.5,
    ...
    print(model.summary())
    return model, 'model_c_cnn_bilstm_CRF#'+'#'.join(features_to_use)+'@conv'+'_'.join([str(c) for c in convs])+'@dense_'+str(dense_size)+'@lstm'+str(lstm_size)+'@droplstm'+str(dropout_rate)+'@filtersize_'+str(filter_size)+ '@lr_' + str(lr)
